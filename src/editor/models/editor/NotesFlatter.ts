...
export class NotesFlatter implements IInstrument {
    ...
    public action = (element: HTMLElement, song: Song) => {
        ...
        if (currentTrack.getNote(cordsX, cordsY)['half'] === noteHalf.NONE) {
            const newNote = calculateHalfNote(currentTrack.getNote(cordsX, cordsY).getSound(), halfMaps.FLAT_MAP)
            currentTrack.getNote(cordsX, cordsY).setSound(newNote, noteHalf.FLAT)
        }
    }
}