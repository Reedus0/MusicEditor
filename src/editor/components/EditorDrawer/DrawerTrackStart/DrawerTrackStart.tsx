...
const DrawerTrackStart: FC<DrawerTrackStartProps> = ({ song, tact, track, tactCounter, tactIndex, trackIndex }) => {
    return (
        <div className='editor-drawer-track-start'>
            {tactCounter === tact.getWidth() || tactIndex === 0 || (song['tacts'][tactIndex - 1] !== undefined && song['tacts'][tactIndex - 1]['tracks'][trackIndex].getClef() !== track.getClef()) || (song['tacts'][tactIndex - 1] !== undefined && JSON.stringify(song['tacts'][tactIndex - 1]['tracks'][trackIndex].getKey()) !== JSON.stringify(track.getKey())) ?
                <div className='editor-drawer-track-start__key-wrapper'>
                    {track.getClef() === clefs.TREBLE ?
                        <div className='editor-drawer-track-start__key _treble'>
                            <img className='editor-drawer-track-start__key-img' width={30} height={80} src={require('./../../../img/treble.png')} />
                        </div>
                        :
                        track.getClef() === clefs.BASS ?
                            <div className='editor-drawer-track-start__key _bass'>
                                <img className='editor-drawer-track-start__key-img _bass' width={40} height={40} src={require('./../../../img/bass.png')} />
                            </div>
                            :
                            <div className='editor-drawer-track-start__key _alto'>
                                <img className='editor-drawer-track-start__key-img _alto' width={40} height={50} src={require('./../../../img/alto.png')} />
                            </div>
                    }
                </div> : <></>}
                ...
                <div className='editor-drawer-track-start__signs' style={{ width: Math.abs(keysHalfsMap.get(track.getKey())) * 17 }}>
                    {[...Array(Math.abs(keysHalfsMap.get(track.getKey())))].map((key: number, index: number) =>
                        keysHalfsMap.get(track.getKey()) < 0
                            ?
                            <h4 className='editor-drawer-track-start__sign'
            ...
            {tactIndex === 0 || (song['tacts'][tactIndex - 1] !== undefined && song['tacts'][tactIndex - 1]['tracks'][trackIndex].getTimeSignature() !== track.getTimeSignature()) ?
                <div className='editor-drawer-track-start__signature'>
                    <div className='editor-drawer-track-start__signautre-number-wrapper'>
                        <h3 className='editor-drawer-track-start__signature-number'>{track.getTimeSignature()[0]}</h3>
                    </div>
                    <div className='editor-drawe-start__signautre-number-wrapper'>
                        <h3 className='editor-drawer-track-start__signature-number'>{track.getTimeSignature()[2]}</h3>
                    </div>
                </div>
                : <></>}
            <div className='editor-drawer-track-start__lines'>
                {[...Array(5)].map(() => <div className='editor-drawer-track-start__line'></div>)}
            </div>
        </div>
    )
}
